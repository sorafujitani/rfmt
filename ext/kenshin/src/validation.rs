...
//! Moved here from lib/kenshin.rb's validate_output! at the phase-6 switchover.

use crate::error::{KenshinError, Result};
...
pub fn validate_output(formatted: &str) -> Result<()> {
    ...
    let line = 1 + bytes[..error.location().start_offset()]
        ...
        .filter(|&&b| b == b'\n')
        .count();
    Err(KenshinError::ValidationError(format!(
        "Formatter produced syntactically invalid output (this is a bug in kenshin, not in your code): {} at line {}",
        error.message(),
        line
    )))
}
...
mod tests {
    ...
    fn rejects_invalid_ruby_with_validation_error() {
        ...
        match err {
            KenshinError::ValidationError(message) => {
                assert!(
                    message.starts_with(
                        "Formatter produced syntactically invalid output (this is a bug in kenshin, not in your code): "
                    ),
