...
//! (`lib/kenshin/prism_bridge.rb`, now only in git history) exactly, so its
...
impl RubyParser for NativeAdapter {
    fn parse(&self, source: &str) -> Result<AstNode> {
        ...
        let errors: Vec<String> = parse_result
            ...
            .collect();
        if !errors.is_empty() {
            // ParseError so lib/kenshin.rb surfaces it as Kenshin::Error
            // "Failed to parse Ruby code: ..." (same shape the pre-migration Ruby bridge raised).
            return Err(KenshinError::ParseError(format!(
                "Parse errors:\n{}",
                errors.join("\n")
            )));
        }
