...
# Check for verbose flag before loading kenshin to set debug mode early
ENV['KENSHIN_DEBUG'] = '1' if ARGV.include?('--verbose')

require 'kenshin'
require 'kenshin/configuration'
require 'kenshin/cache'

module Kenshin
  # Cache management commands
  class CacheCommands < Thor
    desc 'clear', 'Clear all cache data'
    option :cache_dir, type: :string, desc: 'Cache directory (default: ~/.cache/kenshin)'
    def clear
  ...
  # Command Line Interface for kenshin
  class CLI < Thor
    ...
    def version
      say "kenshin #{Kenshin::VERSION}"
      say "Rust extension: #{Kenshin.rust_version}"
    end

    desc 'config', 'Show the effective configuration the formatter will use'
    def config_cmd
      say Kenshin.resolved_config(config_path: options[:config])
    ...
    desc 'init', 'Initialize kenshin configuration'
    option :force, type: :boolean, desc: 'Overwrite existing configuration'
    option :path, type: :string, default: '.kenshin.yml', desc: 'Configuration file path'
    def init
      config_file = options[:path] || '.kenshin.yml'

      # Use Kenshin::Config module for consistent behavior
      result = Kenshin::Config.init(config_file, force: options[:force] || false)
    ...
    def validate_explicit_config!(path)
      ...
      begin
        Kenshin.resolved_config(config_path: path)
      rescue Kenshin::Error => e
        raise Thor::Error, e.message
      end
    end
    ...
    def format_single_file(file)
      ...
      formatted = Kenshin.format(source, config_path: options[:config])
    ...
    def atomic_write(path, content)
      ...
      tmp_path = "#{path}.kenshin-#{Process.pid}.tmp"
