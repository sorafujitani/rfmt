  ...
  module NativeExtensionLoader
    class << self
      ...
      def build_possible_paths
        ...
        paths << File.join(__dir__, 'kenshin')

        # Additional fallback: check for .bundle extension explicitly
        paths << File.join(__dir__, 'kenshin.bundle')
      ...
      def version_specific_path
        File.join(__dir__, ruby_version_dir, 'kenshin')
      end
      ...
      def error_header
        "Unable to load kenshin native extension for Ruby #{RUBY_VERSION}.\n"
      end
      ...
      def workaround_instructions
        <<~MSG.chomp
          Workaround:
          1. Check if kenshin.bundle exists in: #{__dir__}/
          2. If it's in a subdirectory, create a symlink:
             cd #{__dir__}
             ln -sf <subdirectory>/kenshin.bundle kenshin.bundle

          Please report this issue at: https://github.com/sorafujitani/rfmt/issues
        MSG
      end

      # Log debug information if KENSHIN_DEBUG is set
      # @param message [String] message to log
      def debug_log(message)
        return unless ENV['KENSHIN_DEBUG']

        warn "[KENSHIN::NativeExtensionLoader] #{message}"
      end
    end
  end
end
