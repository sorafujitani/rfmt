...
kenshinは[Ruby LSP](https://shopify.github.io/ruby-lsp/)を通じてエディタと統合します。

## 前提条件

1. kenshin gemをインストール:
   ```bash
   gem install kenshin
   ```
...
2. kenshin gemがインストールされているか確認: `gem list kenshin`
3. VSCodeの出力パネルで「Ruby LSP」を選択しログを確認
4. コマンドパレットから「Ruby LSP: Restart」を実行

## 設定

kenshinは`.kenshin.yml`から設定を読み込みます。詳細は[設定ガイド](./configuration.md)を参照してください。
